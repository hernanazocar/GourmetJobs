        ...
        <div className="grid md:grid-cols-3 gap-6 mt-14">
          {howItWorks.map((step, index) => (
            <RevealWrapper key={step.number} delay={index * 0.08}>
              <div className="group relative bg-card rounded-[28px] p-8 border border-border text-left overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:border-orange">
                <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-orange to-orange3 opacity-0 group-hover:opacity-100 transition" />
                <span className="absolute top-4 right-4 text-6xl font-extrabold text-border group-hover:text-orange/20 transition">
                  {step.number}
                </span>
                ...
                <span className="mt-4 inline-block bg-bg2 text-text2 text-xs font-semibold px-3 py-1 rounded-full">
                  {step.chip}
                </span>
              </div>
            </RevealWrapper>
