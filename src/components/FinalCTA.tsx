...
export default function FinalCTA() {
  return (
    <section className="sec-light py-24 px-4 md:px-8">
      <div className="relative z-10 max-w-6xl mx-auto rounded-3xl border border-orange/20 bg-white/90 overflow-hidden py-20 px-6 text-center shadow-2xl shadow-orange/10">
              ...
              <div className="flex -space-x-2">
                {[1,11,5,12,9].map(id => (
                  <div key={id} className="w-8 h-8 rounded-full border-2 border-white overflow-hidden">
                    <img src={"https://i.pravatar.cc/64?img="+id} alt="" className="w-full h-full object-cover" />
                  </div>
                ))}
              </div>
              <span className="text-[#7A5C48] text-sm">+3,200 ya están aquí</span>
            </div>
